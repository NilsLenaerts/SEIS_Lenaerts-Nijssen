...
std::string Compiler::getWasmString(){
...
Bytestream Compiler::compile() {
    ...
    std::string isOk = functionInline();
    if (isOk != ""){
        string = isOk;
        return Bytestream();
    }
...
std::string Compiler::functionInline()
{
	...
	for (Instruction instr : instructions) {
		...
		//if we get the opcode corresponding with call --> we add 1 to the counter.
			...
			if (instr.getStringValue() != "") {
                if (functionCounterTranslation.find(instr.getStringValue()) == functionCounterTranslation.end()){
                    return "called function name not found";
                }
				...
				++(functions.at(functionindex));
                 ...
                 if (functions.find(instr.getParam()) == functions.end()){
                     return "A called function was not defined";
                 }
                else {
...
Bytestream Compiler::convertFunctionsToWASM()
{
    ...
    for (char byte : bytes){
        testString.append( std::to_string((int) byte));
    }
